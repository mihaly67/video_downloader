...
*   **IDŐNKÉNTI HEALTH CHECK:** Szakaszok végén vagy ha úgy érzed a rendszer lassul, futtasd a `python3 ENVIRONMENT_SETUP/agent_health_checker.py` parancsot a terminálban, vagy figyeld a `RAG_SYSTEM/supervisor.log` tartalmát a `tail -n 10 RAG_SYSTEM/supervisor.log` segítségével, ami felvillantja a daemon által adott esetleges emlékeztetőket!

...
*   **A. Sniffer (Hálózati Felderítő - Chrome Extension):** A 403 Forbidden és Sign In hibaüzenetek (YouTube, Xamateur, stb.) miatt a hagyományos Python Playwright Stealth sniffert leváltotta egy natív Manifest V3 böngészőkiegészítő (`src/sniffer/extension/`). Ez a kiegészítő Man-in-the-Middle (MitM) elven felülírja a `window.fetch` és `XMLHttpRequest` hívásokat a felhasználó bejelentkezett, valós böngészőjében, extrahálja az m3u8/stream linkeket, a teljes hitelesített Cookie készletet, majd egy lokális REST API-n keresztül továbbítja azt a Python letöltő motornak (Core).
